...
| RPM install fails: `cpio: mkdir failed` under `/opt` | `/opt` is a symlink to the machine-local `/var/opt`, which does not exist at build time; affects any `/opt` RPM (keybase, Chrome, 1Password) | Create a real `/opt`, install, `mv /opt/<pkg> /usr/lib/opt/<pkg>`, ship an `L /var/opt/<pkg> - - - - /usr/lib/opt/<pkg>` tmpfiles entry, then `rmdir /opt` — see `build/21-keybase.sh` |
...
| Build step fails: `failed pushing cache to ... denied: permission_denied: write_package` | Either the GHCR package is linked to a *different* repo (common after recreating a repo — the old package survives and the new repo's `GITHUB_TOKEN` cannot write to it), or root podman has no registry login for the `--cache-to` push | Delete the stale package (owner → Packages → package → Settings → Delete) or grant the repo Write under *Manage Actions access*; ensure the `Login root podman for registry cache` step runs before `Build image` |
| Promotion workflow fails: `fatal: couldn't find remote ref stable`                     | The `stable` branch was never created during onboarding                                                                                     | `git push origin main:refs/heads/stable`, then protect the branch                                                         |
| Promotion PR fails: `GitHub Actions is not permitted to create or approve pull requests` | Repo setting `can_approve_pull_request_reviews` is false (the default)                                                                      | Settings → Actions → General → check "Allow GitHub Actions to create and approve pull requests", or `gh api -X PUT repos/OWNER/REPO/actions/permissions/workflow -f default_workflow_permissions=read -F can_approve_pull_request_reviews=true` |
| Promotion PR fails requesting a reviewer                                               | Reusable workflow defaults to the `<owner>/maintainers` team, which does not exist on user-owned forks                                      | Pass `request_reviewer: false` (or an explicit `reviewer:`) in `promote-main-to-stable.yml`                               |
